...
namespace CarLister.Controllers
{
    public class CoolCarController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            model.carsList = db.HCL2
                                    .Where(p => p.picurl != null)                    
                                    .OrderByDescending(c => c.model_year)
                                    .ThenBy(c => c.make)
                                    .ThenBy(c => c.model_name)
                                    .ThenBy(c => c.model_trim)
                                    .ToPagedList(1, 9);
                                    //.ToPagedList(1, cars.Count());
        ...
        public ActionResult CarsList(CarViewModel model, string param1, string param2, string param3, int? id, int? page)
        {
            if (param1 != null)
            {
                cars = db.HCL2
                    .Where(p => p.picurl != null)
                    .Where(c => c.model_year == param1);
                sYear = param1;
                if (param2 != null)
                {
                    cars = db.HCL2
                        .Where(p => p.picurl != null)
                        .Where(c => c.make == param2);
                    sMake = param2;
                    if (param3 != null)
                    {
                        cars = db.HCL2
                            .Where(p => p.picurl != null)
                            .Where(c => c.model_name == param3);
                        sModel = param3;
                    }
                }
            }
