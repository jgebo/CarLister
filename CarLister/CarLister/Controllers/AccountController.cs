...
namespace CarLister.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult AssignUserRole(string roleid)
        {
            ...
            TempData["saveuserlist"] = model.Users; //save selection list for POST error checking 
            return View(model);
        }

        //
        // POST: /Account/AssingnUserRole
        [HttpPost]
        [Authorize(Roles = "Administrator")]
        ...
        public ActionResult AssignUserRole(UserRoleViewModel model)
        {
            ...
            {
                ModelState.AddModelError("", "Select a User!");
                model.Users = (MultiSelectList)TempData["saveuserlist"];
                return View(model);

                // another way to handle error
                //TempData["ListError"] = "You must select at least one user from list.";
                //return RedirectToAction("AssignUserRoles", new { roleid = model.RoleId });
            }
        ...
        public ActionResult UnassignUserRole(UserRoleViewModel model)
        {
            ...
            foreach (var id in model.SelectedUsers)
            {
                ApplicationDbContext context = new ApplicationDbContext();
                var userManager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(context));
                userManager.RemoveFromRole(id, model.RoleName);
            }
            return RedirectToAction("ListRoles");
        }
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            //see http://msdn.microsoft.com/en-us/library/01escwtf(v=vs.110).aspx
